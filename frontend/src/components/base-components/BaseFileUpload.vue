<template>
    <div class="p-fileupload p-fileupload-advanced p-component field" v-if="isAdvanced">
      <label class="block text-900 text-0 font-medium mb-2">{{ labelText }}</label>
      <input style="display:none" ref="fileInput" type="file" @change="onFileSelect" :multiple="multiple" :accept="accept" :disabled="chooseDisabled" />
      <div class="p-fileupload-buttonbar" v-if="showButtonBar">
        ...
        <div ref="content" class="p-fileupload-content" @dragenter="onDragEnter" @dragover="onDragOver" @dragleave="onDragLeave" @drop="onDrop">
            ...
            <div class="p-fileupload-files" v-if="hasFiles">
                <div class="p-fileupload-row" v-for="(file, index) of files" :key="file.name + file.type + file.size">
                    <div>
                        ...
                        <i v-else style="font-size: 48px" class="pi pi-file-pdf"/>
                    </div>
                    <div class="p-fileupload-filename">{{file.name}}</div>
                    <div class="p-fileupload-size">{{formatSize(file.size)}}</div>
                    <div>
            ...
            <hr>
            <div class="p-fileupload-files" v-if="uploadedFiles.length">
                <div class="p-fileupload-row uploaded-files" v-for="(file, index) of uploadedFiles" :key="file.id">
                    <div>
                        <Image v-if="isImage(file)" :alt="file.name" :src="$apiFileUrl+file.nameMini" :width="previewWidth" preview @show="openFile(index)" ></Image>
                        <i v-else style="font-size: 48px" class="pi pi-file-pdf"/>
                    </div>
                    <div class="p-fileupload-filename">{{file.filename}}</div>
                    <div class="p-fileupload-size">{{formatSize(file.size)}}</div>
                    <div>
                        <FileUploadButton type="button" icon="pi pi-times" @click="removeUploadedFile(file.id)" />
                    </div>
                </div>
            </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            openFile(index) {
                let img = document.querySelector('.p-image-mask.p-component-overlay.p-component-overlay-enter .p-image-preview')
                img.src = this.$apiFileUrl+this.uploadedFiles[index].filename
            },
            onFileSelect(event) {
            ...
            isImage(file) {
                return /^image\//.test(file.type) || (file.mimetype && file.mimetype.match(/\/(jpg|jpeg|png)$/));
            },
            onDragEnter(event) {
                if (!this.disabled) {
